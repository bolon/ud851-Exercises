...
public class SettingsFragment extends PreferenceFragmentCompat implements
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        try {
            Float f = Float.valueOf((String) newValue);

            if (f > 0 && f <= 3) {
                return true;
            } else throw new RuntimeException("Value must be between 0.1 ~ 3");
        } catch (RuntimeException ex) {
            Toast.makeText(getActivity(), "Value must be between 0.1 ~ 3", Toast.LENGTH_LONG).show();
            return false;
        }
    }
}